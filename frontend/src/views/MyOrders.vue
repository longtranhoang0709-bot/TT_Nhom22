            ...
            <tr v-for="item in selectedOrder.items" :key="item.ma_chi_tiet">
              <td>
                <div class="d-flex align-items-center gap-2">
                   <BImg :src="item.duong_dan_anh ? `http://localhost:3000${item.duong_dan_anh}` : 'https://via.placeholder.com/50'" style="width:40px; height:40px; object-fit:cover" />
                   {{ item.ten_san_pham }}
                </div>
              </td>
              <td>{{ formatPrice(item.don_gia) }}</td>
              <td>x{{ item.so_luong }}</td>
              <td class="fw-bold">{{ formatPrice(item.don_gia * item.so_luong) }}</td>
            </tr>
          </tbody>
        </BTableSimple>

...
import { getAllPromotions, createPromotion, updatePromotion, deletePromotion } from '../../api/promotion';
...
const resetForm = () => {
    form.value = { code: "", discount: 10, minOrder: 0, start: "", end: "", description: "" };
...
<template>
    <div class="p-4">
        ...
        <BRow>
            <BCol md="4">
                <BCard :title="isEditing ? 'Cập nhật mã' : 'Tạo mã mới'" class="shadow-sm mb-4">
                    <BForm @submit.prevent="handleSubmit">
                        <BFormGroup label="Mã Code (VD: SALE50)" class="mb-2">
                            <BFormInput v-model="form.code" required style="text-transform: uppercase"/>
                        </BFormGroup>
                        <BFormGroup label="Giảm (%)" class="mb-2">
                            <BFormInput type="number" v-model="form.discount" min="1" max="100" required />
                        </BFormGroup>
                        <BFormGroup label="Đơn tối thiểu (VND)" class="mb-2">
                            <BFormInput type="number" v-model="form.minOrder" />
                        </BFormGroup>
                        <BRow>
                            <BCol><BFormGroup label="Ngày bắt đầu"><BFormInput type="date" v-model="form.start" required /></BFormGroup></BCol>
                            <BCol><BFormGroup label="Ngày kết thúc"><BFormInput type="date" v-model="form.end" required /></BFormGroup></BCol>
                        </BRow>
                    ...
                    <BTableSimple responsive hover>
                        ...
                        <tbody>
                            <tr v-for="p in promotions" :key="p.ma_khuyen_mai">
                                ...
                                <td><BBadge variant="success">{{ p.giam_phan_tram }}%</BBadge></td>
                                <td>{{ new Intl.NumberFormat('vi-VN').format(p.don_toi_thieu) }}đ</td>
                                <td>
                                ...
                                <td>
                                    <BButtonGroup size="sm">
                                        ...
                                        <BButton variant="danger" @click="handleDelete(p.ma_khuyen_mai)">

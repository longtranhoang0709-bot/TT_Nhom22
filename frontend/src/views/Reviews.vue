  ...
  <div class="review-page py-5">
    <BContainer>
      ...
      <transition name="fade">
        <div
          v-if="showForm"
          class="mb-5 mx-auto p-4 bg-white rounded shadow-sm"
          ...
          <BForm @submit.prevent="handleSubmit">
            <div class="mb-3 text-center">
              <label class="form-label d-block fw-bold"
                >Bạn chấm mấy sao?</label
              >
              <div
                class="rating-stars fs-2 text-warning"
            ...
            <BFormGroup class="mb-3">
              <BFormTextarea
                v-model="content"
                placeholder="Hãy chia sẻ cảm nhận của bạn về đồ uống, không gian..."
                rows="3"
                required
              />
            </BFormGroup>
            <div class="d-flex justify-content-end gap-2">
              <BButton variant="secondary" @click="showForm = false"
                >Hủy</BButton
              >
            ...
            <div class="d-flex justify-content-between align-items-start">
              <div class="d-flex align-items-center gap-3">
                <div class="avatar-circle">
                  {{ r.ho_ten.charAt(0).toUpperCase() }}
                </div>
                <div>
                  <h6 class="fw-bold mb-0">{{ r.ho_ten }}</h6>
                  <small class="text-muted">{{
                    new Date(r.ngay_gui).toLocaleString("vi-VN")
                  }}</small>
                </div>
              </div>
              <div class="text-warning">
                <i
                  v-for="n in 5"
                  :key="n"
                  :class="n <= r.danh_gia ? 'bi bi-star-fill' : 'bi bi-star'"
                ></i>
              </div>
            </div>
